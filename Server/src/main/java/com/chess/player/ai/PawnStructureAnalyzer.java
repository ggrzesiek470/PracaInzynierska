...
public final class PawnStructureAnalyzer {
    ...
    private static Collection<Piece> calculatePlayerPawns(final Player player) {
        return player.getActivePieces().stream().filter(piece ->
                piece.getPieceType() == Piece.PieceType.PAWN).collect(Collectors.toList());
    }
    ...
    private static int[] createPawnColumnTable(final Collection<Piece> playerPawns) {
        ...
        for (final Piece playerPawn: playerPawns) table[playerPawn.getPiecePosition() % 8]++;
