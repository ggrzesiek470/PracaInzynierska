...
//        this.chessBoard = builder.build2();
        ...
        this.pieceIconPath = "Server/AI/art/simple/";
    ...
    public static class AIThinkTank extends SwingWorker<Move, String> {
        ...
        private Alliance computer;
        private Board serverBoard;

        private AIThinkTank() {
            this.depth = Table.get().getGameSetup().getSearchDepth();
        }

        public AIThinkTank(final Long depth, final Board serverBoard, final String computer) {
            this.depth = depth.intValue();
            this.serverBoard = serverBoard;
            if (computer.equals("white"))
                this.computer = Alliance.WHITE;
            else if (computer.equals("black"))
                this.computer = Alliance.BLACK;
        }
            ...
            if (Table.get().getHighlightLegalMoves()) {
                for (final Move move: pieceLegalMoves(board)) {
                    if (move.getDestinationCoordinate() == this.tileId) {
                        try {
                            add(new JLabel(new ImageIcon(ImageIO.read(new File("Server/AI/art/misc/green_dot.png")))));
                        }
