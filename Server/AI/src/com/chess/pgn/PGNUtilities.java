...
public class PGNUtilities {
    ...
    private static List<String> createMovesFromPGN(final String pgnText) {
        ...
        final List<String> sanitizedMoves = new LinkedList<>(Arrays.asList(
                removeParenthesis(pgnText).replaceAll(Pattern.quote("$") + "[0-9]+", "")
                                          .replaceAll("[0-9]+\\s*\\.\\.\\.", "")
                                          .split("\\s*[0-9]+" + Pattern.quote("."))));

        final List<String> processData = removeEmptyText(sanitizedMoves);
        final String[] moveRows = processData.toArray(new String[processData.size()]);
    ...
    private static int deriveCurrentCoordinate(final Board board, final String movedPiece, final String destinationSquare,
